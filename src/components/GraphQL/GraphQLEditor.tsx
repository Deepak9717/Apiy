...
const editorCls =
  'flex-1 w-full min-h-48 bg-zinc-900/50 border border-zinc-700/40 rounded-lg p-3 text-sm text-zinc-200 placeholder:text-zinc-800 focus:outline-none focus:border-pink-500/40 font-mono resize-none transition-colors leading-relaxed'
        ...
        {TABS.map((t) => (
          <button
            ...
            className={`relative px-4 py-2.5 text-sm transition-colors ${
              tab === t.key ? 'text-pink-400' : 'text-zinc-500 hover:text-zinc-300'
            }`}
          >
            {t.label}
            {tab === t.key && (
          ...
          >
            ...
            {tab === 'variables' && (
              <div className="flex flex-col gap-2 h-full">
                <p className="text-xs text-zinc-700">JSON variables passed to the query</p>
                <textarea
                  value={variables}
            ...
            {tab === 'headers' && (
              <div className="flex flex-col gap-2 h-full">
                <p className="text-xs text-zinc-700">Extra request headers as JSON</p>
                <textarea
                  value={headers}
