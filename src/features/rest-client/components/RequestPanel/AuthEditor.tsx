...
import { AuthType } from '@/types/rest'
...
const inputCls =
  'w-full bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700/60 rounded-xl px-3 py-2 text-sm text-zinc-800 dark:text-zinc-200 placeholder:text-zinc-400 dark:placeholder:text-zinc-700 focus:outline-none focus:border-violet-400 dark:focus:border-violet-600/50 font-mono transition-colors'
        ...
        {AUTH_TYPES.map((t) => (
          <button
            ...
            className={`flex flex-col gap-0.5 p-3 rounded-xl border text-left transition-all ${
              auth.type === t.value
                ? 'border-violet-600/50 bg-violet-600/5 text-violet-400'
      ...
      <AnimatePresence mode="wait">
        {auth.type === 'none' && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="flex items-center gap-2 p-3 rounded-xl bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800"
          >
            ...
            {auth.token && (
              <div className="p-2.5 rounded-xl bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
                <p className="text-xs text-zinc-500 dark:text-zinc-600 font-mono break-all">
                  ...
                  <span className="text-violet-600 dark:text-violet-400/70">{auth.token.slice(0, 24)}…</span>
                </p>
              </div>
            )}
          </motion.div>
          ...
          >
            ...
            {auth.username && (
              <div className="p-2.5 rounded-xl bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
                <p className="text-xs text-zinc-500 dark:text-zinc-600 font-mono break-all">
                  Authorization: Basic{' '}
                  <span className="text-violet-600 dark:text-violet-400/70">
            ...
            {auth.apiKeyName && auth.apiKeyValue && (
              <div className="p-2.5 rounded-xl bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
                <p className="text-xs text-zinc-500 dark:text-zinc-600 font-mono">
                  ...
                  <span className="text-violet-600 dark:text-violet-400/70">{auth.apiKeyName}</span>
                  {' = '}
